...
PREDICTION_COUNTER = Counter(
    "tumor_prediction_count", "contador de predicciones por tipo de tumor", ["tumor"]
)
    ...
    try:
        ...
        try:
            ...
            raise FileNotFoundError(
                f"No se encontró el modelo en 'models/{name}.pt' ni en '../models/{name}.pt'"
            )
...
def predict():
    if model is None:
        return (
            jsonify(
                {"error": "Modelo no cargado. Verifica que el archivo .pt existe."}
            ),
            500,
        )
    ...
    try:
        ...
        if os.path.exists(prediction_img_path):
            with open(prediction_img_path, "rb") as img_file:
                images_base64["prediction_plot"] = base64.b64encode(
                    img_file.read()
                ).decode("utf-8")

        if os.path.exists(features_img_path):
            with open(features_img_path, "rb") as img_file:
                images_base64["feature_values_plot"] = base64.b64encode(
                    img_file.read()
                ).decode("utf-8")
        ...
        response = {
            "sample_idx": int(
                explanation_result["sample_idx"]
            ),  # Convertir a int nativo
            "predicted_label": int(
                explanation_result["predicted_label"]
            ),  # Convertir a int nativo
        ...
        if true_label != -1:
            response["true_label"] = int(
                explanation_result["true_label"]
            )  # Convertir a int nativo
